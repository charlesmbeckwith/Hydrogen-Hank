...
public class Player extends GameObject implements Collidable
{
	...
	{
		return new Area(new Ellipse2D.Double(center.getX(), center.getY(),
				width - 5, height - 5));
	...
	private void initDebug()
	{
		...
		String PositionDebug = new String().concat("XPosition: " + (int) x
				+ " || YPosition: " + (int) y);
		String VelocityDebug = new String().concat("XVelocity: "
				+ (int) v.dx);
	...
	{
		private float x, y, rX, rY, xOffset, yOffset, yOffsetTemp;
		...
		public Balloon(float x, float y, int width, int height)
		{
			...
			yOffset = Game.Rand.nextBoolean() ? -Game.Rand.nextFloat()
					: Game.Rand.nextFloat();
		...
		public void tick(float x, float y)
		{
			...
			rY = (int) y - 10 -adjHeight*temp + yOffset;
			dirCounter++;
		}
		...
		public void render(Graphics2D g)
		{
			...
			g.drawLine((int) rX + adjWidth / 2, (int) rY + adjHeight - 5,
					(int) x, (int) y + 10);
