    ...
    extends GameState
{
  ...
  public PlayState()
  {
    ...
    handler.setRemovalConditions(Arrays.asList(RemovalConditions.OffscreenLeft,
                                               ...
                                               RemovalConditions.OffscreenBottom,
                                               RemovalConditions.Dead));
    cam = new Camera(0, 0);
  ...
  private void displayHUD(Graphics2D g)
  {
    ...
    for (int i = 70; i >= percent + 1; i--)
      {
        g.drawImage(art.fulltank_sheet.getFrame(i), tankEnd - (2 * (70 - i)),
                    15, 2, 30, null);
      }
    ...
    renderHelp.outlinedText(g, new Font("Arial", Font.BOLD, 20),
                            (int) (100 * player.getHydrogenLevelPercent())
                                + "%", 0.6f, Color.black, Color.white,
                            tankStart + tankOffset, 36);

    // Draw the Play time
    Date date = new Date((long) (playTime * 1000));
    String formattedDate = new SimpleDateFormat("mm:ss").format(date);
    renderHelp.outlinedText(g, new Font("Arial", Font.BOLD, 28), formattedDate,
                            1.25f, Color.black, Color.white, timerStart, 37);
    ...
    renderHelp.outlinedText(g, new Font("Arial", Font.BOLD, 28), "ALT:"
                                                                 + altitudeStr,
                            1.25f, Color.black, Color.white, altStart, 37);
    ...
    renderHelp.outlinedText(g, new Font("Arial", Font.BOLD, 28),
                            ":" + player.getExtraBalloons(), 1.25f,
                            Color.black, Color.white, balloonsStart + 23, 37);
  }

  /**
   * Clears the necessary objects and reinitializes them to enable a restart of
   * the game
   */
  public void restart()
  {
  ...
  {
    int offset = 75;
    Rectangle sceneWindow = new Rectangle(
                                          (int) (player.getX() - Game.width - offset),
  ...
  private void generateCloud(int x, int y)
  {
    ...
    switch (Game.Rand.nextInt(100))
      ...
      case 2:
        ...
        handler.addObject(new Cloud(x, y, 192, 96, new Vector2D(xVel, 0), true,
                                    false));
        break;
      case 3:
      case 4:
    ...
    for (int t = 0; t < hydrogenCloudSize; t++)
      {
        ...
        handler.addObject(new HydrogenMolecule(x + randX + 10, y + randY, 25,
                                               25));
      }
  }
